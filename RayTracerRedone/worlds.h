...
namespace worlds {
    ...
    World meshTest(){
        ...
        std::vector<std::shared_ptr<VirtualObject>> objects;
        const auto white_matte = std::make_shared<Matte>(1, ColorVec(1, 1, 1));
        auto mesh = std::make_shared<Mesh>("assets/objs/cow.obj", white_matte);
        auto mat = glm::translate(Matrix4x4(1.0f), Vector3(0, -120, -200));
        float angle = 1.5708;
        ...
        std::vector<std::shared_ptr<VectorialLight>> lights;
        //lights.push_back(std::make_shared<DirectionalLight>(Vector3(0, 1, 1), 3.15, ColorVec(1, 1, 1)));
        lights.push_back(std::make_shared<PointLight>(Point3(0, 600, 60), Constants::pi * 60, ColorVec(1, 1, 1)));
        AmbientLight ab(0.3, ColorVec(1.0, 1.0, 1));
