...
class Ball : public VirtualObjectSIMD {
    std::shared_ptr<AABB> aabb;
public:
    Point3 center;
    float radius;

    Ball(Point3 _center, float _radius, std::shared_ptr<Material> _material) : VirtualObjectSIMD(_material),
    ...
    Ball(Point3 _center, float _radius) : VirtualObjectSIMD(), center(_center), radius(_radius) {
        aabb = std::make_shared<AABB>(AABB(
                center - Vector3(radius, radius, radius),
                center + Vector3(radius, radius, radius)));
    };
