...
#include "../scene/materials/NullMaterial.h"
...
std::optional<intersectionRec> Ball::intersects(const Ray& ray) const {
	...
	const float b = glm::dot(origin_minus_center, ray_direction)+ glm::dot(origin_minus_center, ray_direction);
	...
	float t1 = (-b + sqrtf(disc)) * 0.5f * a;
	float t2 = (-b - sqrtf(disc)) * 0.5f * a;
	float closest = 0;
	...
	const Vector3 normal = normalMultiplier*normalize(closestPoint - center);
